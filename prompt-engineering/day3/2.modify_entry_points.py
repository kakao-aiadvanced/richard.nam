...
tavily = TavilyClient(api_key='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')
...
# Define the nodes
workflow.add_node("docs_retrieval", docs_retrieval)  # retrieve
...
app = workflow.compile()
inputs = {"question": "What kind of agent is LLM?"}
for output in app.stream(inputs):
    for key, value in output.items():
        pprint(f"Finished running: {key}:")
